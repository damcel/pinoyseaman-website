  ...
  <table width="80%" border="0" cellpadding="5" cellspacing="5" bgcolor="#FFFFFF">
    <tr>
	  ...
	  if(!isset($offset))
				...
				$query2 = "SELECT * from job_seeker where middle_name like '%$middle_name%' order by last_name,first_name asc" or die("Error" . mysqli_error($link));
				$query = "SELECT * from job_seeker where middle_name like '%$middle_name%' order by last_name,first_name asc limit " . (int)($offset * $rec_per_page) . ",$rec_per_page" or die("Error" . mysqli_error($link));
   ...
   onclick="window.open(this.href,this.target,'width=800,height=500,scrollbars=yes');
		...
		$label = $i + 1;
		?>
        | <a href="search_middlename.php?offset=<?php echo (int)$ctr;?>&middle_name=<? echo $middle_name;?>">
        <?php 
		if($label == $offset+1)
		{
