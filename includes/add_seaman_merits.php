...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $merits = isset($_POST['merits']) ? filter_var(trim($_POST['merits']), FILTER_SANITIZE_STRING) : null;

    // Handle file upload
    $fileUploaded = false;
    if (isset($_FILES['merits_file_upload']) && $_FILES['merits_file_upload']['error'] === UPLOAD_ERR_OK) {
        $uploadDir = "../Uploads/Seaman/Merits/"; // Directory to store uploaded files
        $fileExtension = pathinfo($_FILES['merits_file_upload']['name'], PATHINFO_EXTENSION);
        $uniqueFileName = uniqid() . "_" . time() . "." . $fileExtension; // Generate a unique file name
        $filePath = $uploadDir . $uniqueFileName;

        // Check file type
        $allowedTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];
        if (in_array($_FILES['merits_file_upload']['type'], $allowedTypes)) {
            if (move_uploaded_file($_FILES['merits_file_upload']['tmp_name'], $filePath)) {
    ...
    if ($merits) {
        ...
        $stmt->bind_param("ss", $merits, $seekerEmail);
