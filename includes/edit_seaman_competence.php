...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete']) && $_POST['delete'] == 1) {
        ...
        if ($row) {
            ...
            $filePath = "../Uploads/Seaman/Competence/" . $row['doc_url'];
            ...
            if ($stmt->execute()) {
                ...
                $clearNotesQuery = "UPDATE job_seeker SET competence = NULL WHERE email = ?";
    ...
    $editCompetence = isset($_POST['editCompetence']) ? filter_var(trim($_POST['editCompetence']), FILTER_SANITIZE_STRING) : null;
    ...
    if (isset($_FILES['edit_file_upload']) && $_FILES['edit_file_upload']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileExtension = pathinfo($_FILES['edit_file_upload']['name'], PATHINFO_EXTENSION);
        $uniqueFileName = uniqid() . "_" . time() . "." . $fileExtension; // Generate a unique file name
        $filePath = $uploadDir . $uniqueFileName;

        // Check file type
        $allowedTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];
        if (in_array($_FILES['edit_file_upload']['type'], $allowedTypes)) {
            if (move_uploaded_file($_FILES['edit_file_upload']['tmp_name'], $filePath)) {
    ...
    $stmt->bind_param("ss", $editCompetence, $seekerEmail);

    if (!$stmt->execute()) {
        header("Location: ../competency-certificate.php?type=error&message=Failed to update competence.");
        exit;
    }
    ...
    if ($attachmentUrl) {
        ...
        if ($result->num_rows > 0) {
            // Delete the old file
            $row = $result->fetch_assoc();
            $oldFilePath = "../Uploads/Seaman/Competence/" . $row['doc_url'];
